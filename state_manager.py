# state_manager.py (Corrected - Includes Export Method, DS Handler Refactor, Persistent Last Run Dir, Aug Keys, No Auto Load)
import os
import json
import logging
import torch
...
try:
    ...
    if 'DatasetHandler' not in globals():
        class DatasetHandler:
            ...
            def update_annotation(self, p, d): pass
            def get_annotation(self, p): return None
            def export_for_yolo(self, paths, base_dir, class_map,
                                split=0.2): return None
        logger_sm.warning(
            "Using dummy DatasetHandler defined in state_manager.py")

    if 'TrainingPipeline' not in globals():
        class TrainingPipeline:
            def __init__(self, cl, s, dh): pass
            def cleanup(self): pass
            def update_classes(self, cl): pass
            def update_settings(self, s): pass
            def run_training_session(self, p, a, e, lr, pfx): return None
            def auto_box(self, img, conf): return []
            class_to_id = {}
        logger_sm.warning(
            "Using dummy TrainingPipeline defined in state_manager.py")
...
try:
    ...
    logger_sm.error(
        "Could not import worker classes. Threading will not work.")
    ...
    if 'PredictionWorker' not in globals():
        class PredictionWorker(QObject):
            ...
            def run(self): self.error.emit(
                "Dummy Worker: Prediction unavailable")

            def stop(self): pass
        logger_sm.warning(
            "Using dummy PredictionWorker defined in state_manager.py")
    ...
    if 'TrainingWorker' not in globals():
        class TrainingWorker(QObject):
            ...
            def __init__(self, *args): super().__init__()
            def run(self): self.error.emit(
                "Dummy Worker: Training unavailable")

            def stop(self): pass
        logger_sm.warning(
            "Using dummy TrainingWorker defined in state_manager.py")
...
class StateManager(QObject):
    ...
    def __init__(self, class_list):
        ...
        session_path_key = config.SETTING_KEYS.get(
            "session_path", "paths.session_path")
        session_dir = os.path.dirname(self.get_setting(
            session_path_key, config.DEFAULT_SESSION_PATH))
        if session_dir:
            os.makedirs(session_dir, exist_ok=True)
        self.session_path = self.get_setting(
            session_path_key, config.DEFAULT_SESSION_PATH)  # Store current path
        ...
        try:
            ...
            if 'TrainingPipeline' in globals() and TrainingPipeline.__name__ != '_DummyTrainingPipeline':
                ...
                self.training_pipeline = TrainingPipeline(  # Assumes dummy has same init signature
        ...
        # --- <<< REMOVED Automatic Session Load >>> ---
        # self.load_session() # Load session data, potentially populating last_successful_run_dir
        # --- <<< END REMOVED >>> ---
        ...
        logger_sm.info(
            f"StateManager initialized. Save path: {self.session_path}")
        logger_sm.info(f"User settings path: {self._user_settings_path}")
        # Initialize last_successful_run_dir from settings if available on first run? Or rely only on session load?
        # Let's keep it initialized to None and only populated by load_session or successful training run.
    ...
    def load_settings(self):
        ...
        try:
            if os.path.exists(self._user_settings_path):
                ...
                # Validate and update, don't just blindly merge
                # For simplicity now, just update
                ...
                # Save defaults if file doesn't exist? Optional.
                # self.save_settings()
        except (json.JSONDecodeError, IOError, TypeError) as e:
            logger_sm.error(
                f"Failed load/decode settings from {self._user_settings_path}: {e}. Using defaults.", exc_info=True)
    ...
    def save_settings(self):
        ...
        try:
            os.makedirs(os.path.dirname(
                self._user_settings_path), exist_ok=True)
    ...
    def set_setting(self, key, value):
        ...
        new_value = value  # Start with the input value

        # Attempt type conversion based on known keys
        try:
            int_keys = [config.SETTING_KEYS.get(k) for k in [
                "epochs_20", "epochs_100", "img_size"] if config.SETTING_KEYS.get(k)]
            float_keys = [config.SETTING_KEYS.get(k) for k in [
                "confidence_threshold", "lr_20", "lr_100",
                "aug_flipud", "aug_fliplr", "aug_degrees"  # Added aug keys
            ...
            # Add more type checks if needed (e.g., for paths, booleans)
        except (ValueError, TypeError):
            logger_sm.error(
                f"Invalid type for setting '{key}': '{value}'. Keeping previous value ('{original_value}').")
            return  # Do not update setting if type conversion fails
        ...
        if original_value != new_value:
            ...
            self.update_internal_from_settings(key)  # Update pipeline etc.
            self.settings_changed.emit()  # Notify GUI
        else:
    ...
    def update_internal_from_settings(self, changed_key=None):
        ...
        # Update session path if needed
        session_path_key = config.SETTING_KEYS.get("session_path")
        if session_path_key and (changed_key is None or changed_key == session_path_key):
            self.session_path = self.get_setting(
                session_path_key, config.DEFAULT_SESSION_PATH)

        pipeline_relevant_keys = [
            config.SETTING_KEYS.get(k) for k in [
                "epochs_20", "lr_20", "epochs_100", "lr_100", "img_size",
                "aug_flipud", "aug_fliplr", "aug_degrees"  # Added aug keys
            ] if config.SETTING_KEYS.get(k)  # Filter out None keys
        ]

        if hasattr(self, "training_pipeline") and self.training_pipeline \
           and hasattr(self.training_pipeline, "update_settings"):
            # Update if no specific key changed (initial load) or if the changed key is relevant
            if changed_key is None or changed_key in pipeline_relevant_keys:
                ...
                try:
                    ...
                    logger_sm.error(
                        f"Error updating training pipeline settings: {e_pipe_update}", exc_info=True)

    def get_last_run_path(self):
        """Returns the path to the directory of the last successful training run."""
        return self.last_successful_run_dir

    # --- Core State Methods ---

    def load_session(self, file_path=None):
        session_file = file_path if file_path else self.get_setting(
            # Use current session_path as fallback
        ...
        try:
            ...
            if loaded_run_dir and isinstance(loaded_run_dir, str) and os.path.isdir(loaded_run_dir):
            ...
            if loaded_classes and isinstance(loaded_classes, list):
                ...
                if new_classes != self.class_list:
                    logger_sm.info(
                        f"Updating class list from session: {new_classes}")
            ...
            self.image_list = loaded_images if isinstance(
                loaded_images, list) else []
            self.annotations = loaded_anns if isinstance(
                loaded_anns, dict) else {}

            keys_to_remove = [
                p for p in self.annotations if p not in self.image_list]
            if keys_to_remove:
                logger_sm.warning(
            ...
            if not (isinstance(loaded_index, int) and 0 <= loaded_index < len(self.image_list)):
            ...
            logger_sm.info(f"Session loaded: {len(self.image_list)} images, {len(self.annotations)} annotations. "
    ...
    def save_session(self):
        ...
        session_data = {
            ...
            "class_list": self.class_list,
            "last_successful_run_dir": self.last_successful_run_dir,  # Include in save data
        }
        try:
            ...
            # Consider re-raising if the caller needs to know about the failure
            # raise

    def load_images_from_directory(self, directory_path):
        logger_sm.info(f"Loading images from directory: {directory_path}")
        formats = tuple(f".{ext}" for ext in [
                        "png", "jpg", "jpeg", "bmp", "gif", "tiff", "tif", "webp"])
        try:
            image_files = sorted([
                os.path.abspath(os.path.join(directory_path, f))
                for f in os.listdir(directory_path)
                if os.path.isfile(os.path.join(directory_path, f)) and f.lower().endswith(formats)
            ])
            ...
            logger_sm.error(
                f"Failed load images {directory_path}: {e}", exc_info=True)
            raise
    ...
    def prev_image(self):
        ...
        if self.current_index > 0:
            self.current_index -= 1
            logger_sm.debug(
                f"Navigated to previous index: {self.current_index}")
            return True
    ...
    def go_to_image(self, index):
        if self.image_list and 0 <= index < len(self.image_list):
            if self.current_index != index:
                ...
                logger_sm.debug(
                    f"Navigated directly to index: {self.current_index}")
            return True
        logger_sm.warning(
    ...
    def update_classes(self, new_class_list):
        ...
        if new_classes_clean != self.class_list:
            ...
            for img_path, data in self.annotations.items():
                ...
                if data.get("negative", False):
                    ...
                    if self.dataset_handler:
                        self.dataset_handler.update_annotation(
                            img_path, data)  # Update handler for negatives too
                    continue  # Keep negative images
                ...
                # Decide how to handle image after boxes removed
                new_data = data.copy()  # Always work on a copy
                new_data["annotations_list"] = filtered_boxes
                # If all boxes were removed, maybe mark as negative? Or just keep approved=False?
                # Let's maintain the original approved status for now. User can re-approve if needed.
            ...
            self.approved_count = sum(1 for d in self.annotations.values(
            ) if isinstance(d, dict) and d.get("approved"))
            logger_sm.info(
                f"Approved count after class change: {self.approved_count}")
            self.update_pipeline_classes()
            self.save_session()
            self.settings_changed.emit()
        else:
    ...
    def update_pipeline_classes(self):
        if self.training_pipeline and hasattr(self.training_pipeline, "update_classes"):
            ...
            try:
                ...
                logger_sm.error(
                    "Failed to update pipeline classes.", exc_info=True)
    ...
    def add_annotation(self, image_path, annotation_data):
        ...
        logger_sm.info(
            f"Updating annotation state for {os.path.basename(image_path)}")
        was_approved_before = self.annotations.get(
            image_path, {}).get("approved", False)
        ...
        if is_approved_now and not was_approved_before and self.training_pipeline:
            ...
            trigger_level = None
            if current_approved_count > 0 and current_approved_count % 100 == 0:
                trigger_level = 100
            elif current_approved_count > 0 and current_approved_count % 20 == 0:
                trigger_level = 20
            ...
            if trigger_level == 100:
                ...
                epochs = self.get_setting(
                    epochs_key, config.DEFAULT_EPOCHS_100)
            ...
            if epochs is not None and lr is not None and prefix is not None:
                ...
                QTimer.singleShot(150, lambda e=epochs, l=lr,
                                  p=prefix: self.start_training_task(e, l, p))
    ...
    def start_prediction(self, image_path):
        logger_sm.debug(
            f"Request start prediction for {os.path.basename(image_path)}")
        if "PredictionWorker" not in globals() or not issubclass(globals()['PredictionWorker'], QObject):
            logger_sm.error("PredictionWorker class unavailable or invalid.")
            self.prediction_error.emit(
                "Prediction unavailable (Worker missing).")
            return False
        current_confidence = self.get_setting(
            config.SETTING_KEYS["confidence_threshold"])
        return self._start_task(globals()['PredictionWorker'], image_path, current_confidence)

    def start_training_task(self, epochs, lr, run_name_prefix):
        if "TrainingWorker" not in globals() or not issubclass(globals()['TrainingWorker'], QObject):
            logger_sm.error("TrainingWorker class unavailable or invalid.")
            self.training_error.emit("Training unavailable (Worker missing).")
            return False

        logger_sm.info(
            f"Preparing data for training run '{run_name_prefix}'...")
        approved_annotations = {p: data for p, data in self.annotations.items(
        ...
        return self._start_task(globals()['TrainingWorker'], approved_paths, approved_annotations, epochs, lr, run_name_prefix)

    def _start_task(self, worker_class, *args):
        task_name = worker_class.__name__
        if not self.training_pipeline or not hasattr(self.training_pipeline, 'run_training_session'):
        ...
        if self._blocking_task_running:
            logger_sm.warning(
                f"Cannot start {task_name}: Another task running.")
            if task_name == "PredictionWorker":
        ...
        try:
            ...
            if isinstance(self._current_worker, PredictionWorker):
                ...
                self._current_worker.finished.connect(
                    lambda result=None, worker=self._current_worker: self._on_task_finished(worker.__class__.__name__, result))
                self._current_worker.error.connect(
                    lambda worker=self._current_worker: self._on_task_finished(worker.__class__.__name__, None))
            elif isinstance(self._current_worker, TrainingWorker):
                self._current_worker.progress.connect(self.training_progress)
                self._current_worker.finished.connect(
                    self.training_run_completed)
    ...
    def _on_task_finished(self, task_name, result=None):
        ...
        # Update last successful run dir ONLY if it was a Training task AND result is a valid path
        if task_name == "TrainingWorker" and isinstance(result, str) and os.path.isdir(result):
            ...
            logger_sm.debug(
                f"Prediction task finished with {len(result)} results.")
        elif result is None:
            logger_sm.warning(
                f"{task_name} task finished with error or no result.")
        ...
        # Reset state only if the currently finished task was the blocking one
        if self._blocking_task_running:
            self._blocking_task_running = False
    ...
    def cleanup(self):
        ...
        if self._blocking_task_running and self._current_worker and hasattr(self._current_worker, "stop"):
            ...
            try:
                ...
                logger_sm.error(
                    f"Error signaling worker ({worker_name}) stop: {e}")
    ...
    def export_data_for_yolo(self, target_dir):
        ...
        if not hasattr(self.dataset_handler, "export_for_yolo"):
            ...
            return None
        if not self.training_pipeline or not hasattr(self.training_pipeline, "class_to_id"):
            logger_sm.error(
                "Cannot export: Training pipeline or class_to_id map missing.")
            return None

        approved_paths = [p for p, data in self.annotations.items(
        ) if isinstance(data, dict) and data.get("approved")]
        if not approved_paths:
        ...
        if not export_annotations:
            logger_sm.error(
                "No valid annotation data found for approved paths.")
            return None
        ...
        try:
            ...
            yaml_path = self.dataset_handler.export_for_yolo(image_paths_to_export=list(
                export_annotations.keys()), base_export_dir=target_dir, class_to_id=class_to_id, val_split=0.0)
        except Exception as e:
            logger_sm.exception(
            ...
            if original_handler_anns is not None and self.dataset_handler:
                ...
                logger_sm.error(
                    "Cannot restore annotations: DatasetHandler is None.")
        return yaml_path
